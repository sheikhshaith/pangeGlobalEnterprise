...
const TestimonialSlider = () => {
  ...
  const TestimonialCard = ({ testimonial }) => (
    <div className="relative bg-gray-800 p-6 lg:p-8 rounded-2xl shadow-xl text-white flex flex-col lg:flex-row gap-4 lg:gap-6 items-center w-full h-full">
      ...
      <div className="flex-shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-2 border-[#C59740] md:-mt-8">
        <img
      ...
      <div className="flex-1 w-full">
        ...
        <div className="flex items-center mb-2 md:mb-4 justify-center md:justify-start">
          {[...Array(testimonial.rating)].map((_, i) => (
            <svg
              key={i}
              className="w-4 h-4 md:w-5 md:h-5 text-[#C59740] fill-current"
              viewBox="0 0 20 20"
            >
      ...
      <div className="relative w-full max-w-6xl px-2 md:px-4">
        ...
        <div className="flex justify-between items-center mt-4 md:mt-8">
          <div className="flex items-center gap-2 md:gap-4">
            <button
              onClick={prevSlide}
              className="text-white bg-[#C59740] rounded-full p-1 md:p-2 shadow-lg hover:bg-[#C59740] text-sm md:text-base"
            >
              &#10094;
            </button>
          </div>
            ...
            {Array.from({ length: totalSlides }).map((_, index) => (
              <button
                ...
                className={`w-2 h-2 md:w-3 md:h-3 rounded-full transition-colors ${
                  index === currentSlide ? "bg-[#C59740]" : "bg-gray-600"
                }`}
              />
            ))}
          </div>
